...
.. important:: These steps are not applicable to the roboRIO 2. For the roboRIO 2.0,` the microSD card must be imaged with a computer and SD burner application using :doc:`roboRIO 2.0 microSD card imaging process</docs/software/roborio-info/roborio2-imaging>`; imaging the roboRIO 2 directly using the roboRIO Imaging Tool is not supported.
