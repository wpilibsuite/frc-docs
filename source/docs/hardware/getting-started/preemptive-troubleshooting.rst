...
Make sure the correct barrel jack is used, not one that is too small and falls out for no reason.  This isn’t common, but ask an FTA and every once in awhile and they will tell you that some team will use a random barrel jack that is not sized correctly, and it falls out in a match on first contact.
