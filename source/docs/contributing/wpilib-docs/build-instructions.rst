...
This document contains information on how to build the HTML, PDF, and EPUB versions of the wpilib-docs site. wpilib-docs uses Sphinx as the documentation generator. This document also assumes you have basic knowledge of [Git](https://git-scm.com/) and console commands.
...
By default, the reStructuredText extension enables linting with all doc8 features enabled. As wpilib-docs does not follow the line length lint, add the following to your VS Code ``settings.json`` to disable line length linting.
...
.. important:: After modifying wpilib-docs dependencies in any way, ``requirements.txt`` must be regenerated by running ``uv export --frozen --no-dev --no-editable -o source/requirements.txt`` from the root of the repo.
