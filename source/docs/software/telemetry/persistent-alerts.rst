...
Instead, the ``Alert`` class ([Java](https://github.wpilib.org/allwpilib/docs/beta/java/edu/wpi/first/wpilibj/Alert.html), [C++](https://github.wpilib.org/allwpilib/docs/beta/cpp/classfrc_1_1_alert.html), :py:class:`Python <robotpy:wpilib.Alert>`) can be used for managing persistent alerts published via Network Tables. Alerts are assigned a priority (*error*, *warning*, or *info*) and can be *activated* or *deactivated* in robot code. The set of active alerts can be displayed on a dashboard, as shown below in Shuffleboard.
