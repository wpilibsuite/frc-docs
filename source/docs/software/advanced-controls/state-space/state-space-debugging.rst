...
Reliable data of the :term:`system's <system>` :term:`state`\s, :term:`input`\s and :term:`output`\s over time is important when debugging state-space controllers and observers. One common approach is to send this data over NetworkTables and log it with :doc:`DataLog </docs/software/telemetry/datalog>`, then use tools like :doc:`/docs/software/dashboards/advantagescope`, which allows us to both graph the data in real-time as well as export the log to a CSV file for plotting later with tools such as Google Sheets, Excel or Python.
