...
Debouncing is implemented in WPILib by the ``Debouncer`` class ([Java](https://github.wpilib.org/allwpilib/docs/beta/java/org/wpilib/math/filter/Debouncer.html), [C++](https://github.wpilib.org/allwpilib/docs/beta/cpp//classwpi_1_1math_1_1_debouncer.html), :external:py:class:`Python <wpimath.filter.Debouncer>`), which filters a boolean stream so that the output only changes if the input sustains a change for some nominal time period.
