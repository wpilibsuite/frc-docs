...
These new left and right velocities are still speeds and not voltages, so two PID Controllers, one for each side, should be used to track them. You can use either the WPILib PIDController ([C++](https://github.wpilib.org/allwpilib/docs/2027/cpp/classwpi_1_1math_1_1_p_i_d_controller.html), [Java](https://github.wpilib.org/allwpilib/docs/2027/java/org/wpilib/math/controller/PIDController.html), :external:py:class:`Python <wpimath.controller.PIDController>`) or the Velocity PID feature on smart motor controllers such as the TalonSRX and the SPARK MAX.
