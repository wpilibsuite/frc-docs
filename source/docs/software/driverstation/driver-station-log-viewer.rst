...
.. note:: Several alternative tools exist that provide similar functionality to the FRC Driver Station Log Viewer. :ref:`AdvantageScope <docs/software/telemetry/advantagescope:AdvantageScope>` is an option included in WPILib, and `DSLOG Reader <https://github.com/orangelight/DSLOG-Reader>`__ is a third-party option. Note that WPILib offers no support for third-party projects.
