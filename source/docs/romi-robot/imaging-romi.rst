...
The Raspberry Pi firmware is based on the WPILibPi (formerly FRCVision) and must be downloaded and written to the Raspberry Pi micro SSD card. Information about the Raspberry Pi image can be found here. Click on ``Assets`` at the bottom of the description to see the available image files:
