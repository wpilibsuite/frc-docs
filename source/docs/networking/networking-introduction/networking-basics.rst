...
If using the USB interface, no network setup is required (you do need the :doc:`FRC Game Tools </docs/zero-to-robot/step-2/frc-game-tools>` installed to provide the roboRIO USB Driver). The roboRIO driver will automatically configure the IP address of the host (your computer) and roboRIO and the software listed above should be able to locate and utilize your roboRIO.
